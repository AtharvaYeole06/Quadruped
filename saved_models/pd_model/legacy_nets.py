...
class Net(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.base = base_type(activation_func=activation_func, add_ln=add_ln, **kwargs)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for idx in range(head_num):
            ...
            self.__setattr__("head{}".format(idx), head)
...
class NatureEncoderProjNet(nn.Module):
    ...
    def forward(self, x):
        ...
        visual_input = x.view(torch.Size(x.shape[:-1] + self.visual_input_shape))
...
class ImpalaFuseResidualActor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        visual_append_input_shape = (
            self.encoder.visual_dim + self.encoder.base.output_shape
        )
    ...
    def forward(self, x):
        ...
        if self.state_detach:
            out = torch.cat([visual_out, state_out.detach()], dim=-1)
        else:
            out = torch.cat([visual_out, state_out], dim=-1)
        out = self.visual_seq_append_fcs(out)
...
class LocoTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.second:
            out_second = out[
                2 + self.per_modal_tokens : 2 + 2 * self.per_modal_tokens, ...
            ]
            if self.max_pool:
                out_second = out_second.max(dim=0)[0]
            else:
